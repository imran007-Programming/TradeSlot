...
import {
  ...
  Clock, BarChart3, DollarSign, TrendingUp, ExternalLink, Car, ShieldCheck
} from 'lucide-react';

import { Message, Booking, WorkArea, Conversation, Slot } from '@/types';
...
export default function DashboardPage() {
  ...
  const handleResetStripe = () => {
    toast('Reset your Stripe connection?', {
      action: { label: 'Reset', onClick: async () => {
        try {
  ...
  const handleCreateBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        await refreshAll(); toast.success('Slot proposal sent to customer! Awaiting customer confirmation.');
      } else {
    ...
    toast('Delete this work area?', {
      action: { label: 'Delete', onClick: async () => {
        try {
          ...
          if (res.success) { toast.success('Work area deleted!'); await fetchWorkAreas(); }
          else toast.error(res.message || 'Failed to delete work area');
        } catch (err: any) { toast.error(err.message); }
      }},
      cancel: { label: 'Cancel', onClick: () => {} },
    });
    ...
    <div className="h-screen w-screen bg-slate-50 text-slate-800 flex flex-col overflow-hidden" style={{ fontFamily: 'Poppins, system-ui, sans-serif' }}>
      ...
      <header className="h-14 bg-white border-b border-slate-200 px-5 flex items-center justify-between flex-shrink-0 z-30 shadow-sm">
        <div className="flex items-center space-x-3">
          <Link href="/" className="w-8 h-8 rounded-xl bg-[#E11D48] flex items-center justify-center shadow-md shadow-[#E11D48]/20">
            <Zap size={15} className="text-white fill-white" />
          </Link>
          <h1 className="text-sm font-bold text-[#0F172A] tracking-tight">Trade<span className="text-[#E11D48]">Slot</span></h1>
          <span className="text-[10px] bg-[#FFF1F2] text-[#E11D48] border border-[#E11D48]/30 px-1.5 py-0.5 rounded-md uppercase tracking-widest font-bold">Pro</span>
        </div>
        <div className="flex items-center space-x-2">
          <button onClick={() => setShowWorkAreaModal(true)} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200 transition flex items-center gap-1.5 cursor-pointer">
          ...
          {stripeStatus.onboardingComplete ? (
            ...
            <button onClick={handleConnectStripe} disabled={connectingStripe} className="bg-[#E11D48] hover:bg-[#BE123C] text-white px-3.5 py-1.5 rounded-lg text-xs font-bold disabled:opacity-50 transition shadow-sm flex items-center gap-1.5 cursor-pointer">
        ...
        <aside className="w-52 bg-white border-r border-slate-200 flex flex-col justify-between flex-shrink-0 h-full py-5 px-3 shadow-sm">
            ...
            {navItems.map(({ tab, icon, label, count }) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  activeTab === tab ? 'bg-[#FFF1F2] text-[#E11D48] border border-[#E11D48]/30 shadow-xs' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50 border border-transparent'
                }`}>
                {icon} <span>{label}</span>
                {count > 0 && <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded-md font-bold border bg-white text-[#E11D48] border-[#E11D48]/20">{count}</span>}
              </button>
            ))}
          </div>
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2">
            <div className="flex justify-between items-center">
              ...
              <strong className="text-xs text-[#E11D48] font-bold">30 min</strong>
            </div>
            <div className="flex justify-between items-center">
          ...
          <div className="grid grid-cols-5 gap-3 flex-shrink-0">
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-0.5 text-[#0F172A]">{conversations.length}</h3>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-0.5 text-emerald-600">{paymentSummary.succeededCount}</h3>
                <p className="text-[10px] text-slate-400 mt-0.5">{paymentSummary.succeededCount} of {bookings.length} paid</p>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-0.5 text-amber-600">${paymentSummary.totalPending.toFixed(2)}</h3>
                <p className="text-[10px] text-amber-600/90 font-bold mt-0.5">{paymentSummary.pendingCount} unpaid jobs</p>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-0.5 text-emerald-600">${paymentSummary.totalEarned.toFixed(2)}</h3>
                <p className="text-[10px] text-emerald-600/80 font-bold mt-0.5">{paymentSummary.succeededCount} paid</p>
              </div>
              <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center border border-emerald-100">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <h3 className="text-sm font-bold mt-0.5 text-[#0F172A]">{stripeStatus.onboardingComplete ? 'Active' : 'Not Setup'}</h3>
                <p className="text-[10px] text-slate-400 mt-0.5">{stripeStatus.onboardingComplete ? 'Direct payout' : 'Action needed'}</p>
              </div>
                  ...
                  {filteredConversations.length === 0 ? (
                    ...
                    return (
                      <div key={conv.id} onClick={() => setSelectedConversation(conv)}
                        className={`w-full text-left p-3.5 transition-all cursor-pointer ${isSelected ? 'bg-[#FFF1F2] border-l-4 border-[#E11D48]' : 'hover:bg-slate-50 border-l-4 border-transparent'}`}>
                ...
                <table className="w-full text-left">
                  <thead className="text-slate-400 font-semibold uppercase tracking-wider border-b border-slate-100 text-[10px] sticky top-0 bg-white">
                    <tr>
                      ...
                      <th className="p-3">Fee</th><th className="p-3">Payment</th><th className="p-3">Status</th><th className="p-3">Action</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {bookings.length === 0 ? (
                      <tr><td colSpan={7} className="p-8 text-center text-slate-400 text-xs">No bookings found</td></tr>
                    ) : bookings.map(b => (
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {allCustomers.length === 0 ? (
                      ...
                      <tr key={cust.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded-md font-mono text-[10px] font-bold border ${cust.channel === 'WHATSAPP' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
                            {cust.channel}
                          </span>
                        </td>
              ...
              <div className="overflow-y-auto flex-1 mt-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {workAreas.length === 0 ? (
                    ...
                    <div key={wa.id} className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2 relative hover:border-[#E11D48] hover:shadow-md transition-all">
