...
import {
  ...
  RefreshCw, CreditCard, Clock, BarChart3, DollarSign
} from 'lucide-react';
...
export default function DashboardLayout({
  ...
  const refreshLayoutData = async () => {
    try {
      const [userRes, stripeRes, paymentRes, convRes, bookRes, waRes] = await Promise.allSettled([
      ...
      if (stripeRes.status === 'fulfilled' && stripeRes.value.success && stripeRes.value.data) {
        setStripeStatus({
      ...
      if (waRes.status === 'fulfilled' && waRes.value.success && waRes.value.data) {
  ...
  const checkStripe = async () => {
    try {
      ...
      if (res.success && res.data) {
        setStripeStatus({ connected: res.data.connected, onboardingComplete: res.data.onboardingComplete });
      }
    } catch {}
  };
  ...
  const navItems = [
    { href: '/dashboard/messages', icon: <MessageSquare size={15} />, label: 'Messages', count: intakesCount },
    { href: '/dashboard/bookings', icon: <CalendarDays size={15} />, label: 'Bookings', count: bookingsCount },
    { href: '/dashboard/customers', icon: <Users size={15} />, label: 'Customers', count: customersCount },
    { href: '/dashboard/workareas', icon: <MapPin size={15} />, label: 'Work Areas', count: workAreasCount },
  ];
          ...
          <div className="grid grid-cols-5 gap-3 flex-shrink-0">
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-0.5 text-[#0F172A]">{intakesCount}</h3>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">{paymentSummary.succeededCount} of {bookingsCount} paid</p>
              </div>
