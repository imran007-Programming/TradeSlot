...
import { MessageSquare, Send, CalendarDays, CreditCard, Trash2, CheckCircle, Clock, ExternalLink, Car, ShieldCheck } from 'lucide-react';
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-sm text-[#0F172A]">{conversation.customer.name}</h3>
              <span
            ...
            <div className="bg-emerald-50/60 border-b border-emerald-200/80 px-4 py-3 flex-shrink-0">
              <div className="flex items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-emerald-200 shadow-xs">
                ...
                <button
                  ...
                  className="bg-[#0F172A] hover:bg-[#1E293B] active:scale-98 text-white text-xs font-bold px-3.5 py-2 rounded-xl transition disabled:opacity-50 flex items-center gap-1.5 shadow-xs whitespace-nowrap cursor-pointer flex-shrink-0"
                >
                  <CreditCard size={13} />
          ...
          conversation.messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex items-end gap-2 ${isCustomer ? 'justify-start' : 'justify-end'}`}>
                ...
                {m.content.toLowerCase().startsWith('booking confirmed:') || m.content.toLowerCase().startsWith('booking offer:') ? (
                  (() => {
                  ...
                  const isPayment = (m.content.toLowerCase().includes('payment link') || m.content.toLowerCase().includes('stripe.com')) && !!url;
                  ...
                  return (
                    ...
                    >
                      ...
                      {m.sentAt && (
                        <span className={`text-[9px] block text-right mt-1 ${isCustomer ? 'text-slate-400' : 'text-white/80'}`}>
